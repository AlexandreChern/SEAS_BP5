...
fNy_start = (N_x - BP5_coeff.lf) / (2 * BP5_coeff.Δz / 1000) + 1
...
fN2_VW_favorite = BP5_coeff.w / (BP5_coeff.Δz / 1000) + 1
fN2_VW_favorite = round(Int, fN2_VW_favorite, RoundUp)
...
fNy_VW_start = (N_x - BP5_coeff.l) / (2 * BP5_coeff.Δz / 1000) + 1
fNy_VW_start = round(Int, fNy_VW_start, RoundUp)
fNy_VW = (fNy_VW_start, fNy_VW_start + fN2_VW - 1)
fNy_VW_favorite = (fNy_VW_start, fNy_VW_start + fN2_VW_favorite - 1)
...
fNy_VW_VS_start = (N_x - BP5_coeff.l - BP5_coeff.ht) / (2 * BP5_coeff.Δz / 1000) + 1
...
SBPp = 2                # SBPp order
M, RHS, H_tilde, HI_tilde, analy_sol, source, traction_operators, u_filters, Face_operators, sigmas, updators = Assembling_3D_matrices(N_x, N_y, N_z;p=SBPp);
...
function get_favorite_indices(Nx, Ny, Nz, fNy_VW_favorite, fNz_VW)
    x_idx = spzeros(Nx)
    y_idx = spzeros(Ny)
    z_idx = spzeros(Nz)
    x_idx[1] = 1 # the fault is on the first face
    y_idx[fNy_VW_favorite[1]:fNy_VW_favorite[2]] .= 1
    z_idx[fNz_VW[1]:fNz_VW[2]] .= 1
    return kron(z_idx, y_idx, x_idx)
end
